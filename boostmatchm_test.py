...
df = pd.read_csv(pathname + 'boostmatchm_2.csv')
# Get dummies
...
df = df.drop('Cross.group_id', 1)
# Train/test split
np.random.seed(7)
# Declare y,X
y = df['Cross.tdum']
X = df.drop('Cross.tdum', 1)
print(X.head(5))
print("check1")
...
params = {'bagging_fraction': 0.7790529005814426, 'bagging_freq': 35, 'boosting_type': 'gbdt', 'feature_fraction': 0.837776503392061, 'lambda_l1': 0.7000000000000001, 'learning_rate': 0.042490202132117696, 'max_bin': 5, 'max_depth': 1, 'min_data_in_leaf': 1, 'min_sum_hessian_in_leaf': 0, 'num_leaves': 48, 'objective': 'binary', 'verbose': -1,
 #       'objective' :'binary',
#        'learning_rate' : 0.19645814912436993,#0.58,#0.98, #0.97, 
#        'max_depth' : 2,#-1,
#        'lambda_l1': 0.8,#0.91,#0.18,#0.01,
#        'num_leaves': 39,#30,
#        'feature_fraction': 0.6137796130952973,#0.9,#0.91, #0.97
#        'bagging_fraction': 0.3979915887985676,#0.65, #0.65
#        'max_bin': 5,#10,
#        'bagging_freq': 22,#9, #9
...
#with open('AUCs7m.csv', 'w', newline='') as f:
 #   writer = csv.writer(f)
#    writer.writerows(zip(I,kay,A,mini,maxi))
 #   writer.writerows(zip(cols,imp))
